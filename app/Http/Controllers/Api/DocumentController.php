...
class DocumentController extends Controller
{
    ...
    public function reject(Request $request, $id)
    {
        try {
            ...
            $document->update([
                ...
                'is_verified' => false,
                'rejection_reason' => $this->sanitizeInput($validated['reason']),
                'rejected_at' => now(),
                'rejected_by' => $user->id,
            ]);
            ...
            return response()->json(['success' => false, 'message' => 'Failed to reject document'], 500);
        }
    }
    ...
    public function analytics()
    {
        try {
            ...
            $analytics = [
                ...
                'verified_documents' => Document::where('is_verified', true)->count(),
                'pending_documents' => Document::where('is_verified', false)->whereNull('rejected_at')->count(),
                'rejected_documents' => Document::whereNotNull('rejected_at')->count(),
                'documents_with_aadhaar' => Document::whereNotNull('aadhaar_image')->count(),
                'documents_with_license' => Document::whereNotNull('license_image')->count(),
                'verification_rate' => $this->getVerificationRate(),
                'average_verification_time' => $this->getAverageDocumentVerificationTime(),
            ];
    ...
    private function getVerificationRate(): float
    {
        ...
        $verified = Document::where('is_verified', true)->count();
    ...
    {
        $verifiedDocs = Document::whereNotNull('verified_at')->get();

        if ($verifiedDocs->isEmpty()) {
            return null;
        }

        $totalMinutes = $verifiedDocs->sum(function ($doc) {
            return $doc->created_at->diffInMinutes($doc->verified_at);
        });
