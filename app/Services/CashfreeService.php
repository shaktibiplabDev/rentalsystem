...
class CashfreeService
{
     ...
     */
    public function verifyDrivingLicense($dlNumber, $dob)
    {
        try {
            $verificationId = 'DL_' . uniqid() . '_' . date('YmdHis');

            Log::info('Starting driving license verification', [
                'dl_number' => $dlNumber,
            ...
            if ($response->successful()) {
                ...
                Log::info('Driving license verification successful', [
                    'dl_number' => $dlNumber,
            ...
            Log::error('Driving License Verification failed', [
                'dl_number' => $dlNumber,
                'status' => $response->status(),
                'response' => $response->body()
            ]);
            ...
            Log::error('Driving License Verification exception', [
                'dl_number' => $dlNumber,
                'error' => $e->getMessage()
            ]);
